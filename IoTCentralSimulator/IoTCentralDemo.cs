    ...
    public class IoTCentralDemo
    {
        ...
        public string MakeRequest(string method, string payload, string authScope, string address, bool isDPS)
        {
            ...
            else
            {
                ...
                request.Headers.Add("TelemetryInterval", "10");
            }
        ...
        public void Run(int numberOfMessage = 1)
        {
            ...
            for (int i = 0; i < numberOfMessage; i++)
            {
                ...
                json = MakeRequest("POST", "{\"MessageTime\" : \"" + messageTime + "\",\"Moisture\":" + randMoisture + "}", hostName, address, false);

                if (json.Length == 0)
                {
                    Console.WriteLine("Success!! Check the new Moisture telemetry on Azure IoT Central Device window");
                }
                else
                {
